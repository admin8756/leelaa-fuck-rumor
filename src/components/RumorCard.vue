<template>
  <div class="card relative mb-4 sm:mb-8 transform hover:scale-[1.01] transition-all duration-200 overflow-hidden"
    :class="{
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="mb-3 sm:mb-4 border-b pb-2 sm:pb-3" :class="{ 'border-gray-200 dark:border-gray-700': true }">
        <h3 class="text-lg sm:text-xl font-bold text-blue-800 dark:text-blue-300"
      ...
      <div class="flex-grow mb-4 sm:mb-6">
        <p class="text-gray-700 dark:text-gray-300 whitespace-pre-line" :class="{
      ...
      <!-- Actions Section -->
      <div class="flex flex-wrap justify-end gap-2 sm:space-x-3 mt-auto pt-2 border-t"
        :class="{ 'border-gray-200 dark:border-gray-700': true }">
        <button @click="handleReadAloud"
          class="btn btn-secondary flex items-center transition-all relative text-sm sm:text-base" :class="{
      ...
      'text-blue-700 dark:text-blue-300': isCurrentRumor
          ...
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-1 sm:mr-2" fill="none"
            ...
            <path v-if="!isCurrentRumor || (isCurrentRumor && textToSpeech.isPaused.value)" stroke-linecap="round"
          ...
          <!-- 语音波动画效果 -->
          <div v-if="isCurrentRumor && !textToSpeech.isPaused.value"
        ...
        <button @click="copyContent" class="btn btn-secondary flex items-center transition-all text-sm sm:text-base"
          :class="{
      'text-base sm:text-lg px-3 sm:px-5 py-2 sm:py-3': theme === 'elderly',
    ...
    <!-- 卡片角落的音频指示器 -->
    <div v-if="isCurrentRumor && !textToSpeech.isPaused.value" class="audio-indicator" aria-label="正在朗读">
